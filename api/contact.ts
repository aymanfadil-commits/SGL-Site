...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    await resend.emails.send({
      ...
      replyTo: email.trim(),
      subject: `New Mandate Enquiry — ${name.trim()} (${organisation.trim()})`,
      html: `
        <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a1a; background: #ffffff;">
          <div style="background: #09111F; padding: 36px 40px;">
            ...
            <h1 style="color: #F0E9DB; font-size: 22px; margin: 0; font-weight: 400; letter-spacing: 1px;">New Mandate Enquiry</h1>
          </div>
          <div style="padding: 40px;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                <td style="padding: 14px 0; border-bottom: 1px solid #eeeeee; color: #888; font-size: 11px; text-transform: uppercase; letter-spacing: 2px; vertical-align: top;">Organisation</td>
                <td style="padding: 14px 0; border-bottom: 1px solid #eeeeee; font-size: 15px;">${organisation.trim()}</td>
              </tr>
              <tr>
              ...
              <tr>
                <td style="padding: 14px 0; color: #888; font-size: 11px; text-transform: uppercase; letter-spacing: 2px; vertical-align: top; padding-top: 20px;">Mandate</td>
                <td style="padding: 14px 0; font-size: 15px; line-height: 1.7; padding-top: 20px;">${mandate.trim().replace(/\n/g, '<br>')}</td>
              </tr>
            </table>
    ...
    return res.status(500).json({
      error: 'Failed to send enquiry. Please email us directly at info@sagegloballinks.com.',
    });
  }
}
